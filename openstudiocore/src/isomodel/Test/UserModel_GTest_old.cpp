...
TEST_F(ISOModelFixture, UserModel)
{
  ...
  */
  ...
  // TODO: update ISOModel to accept openstudio path objects to avoid having to .string() everything. BAA@2015-08-18
  auto weatherFilePath = resourcesPath() / openstudio::toPath("isomodel/weather.epw");
  userModel.setWeatherFilePath(weatherFilePath.string());
  EXPECT_EQ(userModel.weatherFilePath(), weatherFilePath.string());
  userModel.loadWeather();

  auto wd = userModel.weatherData();
