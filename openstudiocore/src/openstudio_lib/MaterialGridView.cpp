...
MaterialGridView::MaterialGridView(bool isIP, const model::Model & model, QWidget * parent)
  : QWidget(parent),
    m_isIP(isIP)
{
  ...
  auto spaceTypesGridController = new MaterialGridController(m_isIP, "Space Types", IddObjectType::OS_SpaceType, model, spaceTypeModelObjects);
  ...
  isConnected = connect(m_filters, &QComboBox::currentTextChanged, m_gridController, &openstudio::MaterialGridController::filterChanged);
...
std::vector<model::ModelObject> MaterialGridView::selectedObjects() const
{
...
void MaterialGridView::enableFilter()
{
  m_filterLabel->setEnabled(true);
  m_filters->setEnabled(true);
}

void MaterialGridView::disableFilter()
{
  m_filterLabel->setEnabled(false);
  m_filters->setEnabled(false);
}

void MaterialGridView::onDropZoneItemClicked(OSItem* item)
{
}

MaterialGridController::MaterialGridController(bool isIP,
...
void MaterialGridController::setCategoriesAndFields()
{
...
void MaterialGridController::filterChanged(const QString & text)
{
...
MaterialGridView * MaterialGridController::materialGridView()
{
  auto gridView = qobject_cast<OSGridView *>(this->parent());
  OS_ASSERT(gridView);

  auto materialGridView = qobject_cast<MaterialGridView *>(gridView->parent());

  return materialGridView;
}

void MaterialGridController::categorySelected(int index)
{
  auto gridView = this->materialGridView();
  if (gridView) {
  ...
  if (materialGridView()) {
    ...
    // ...or if the custom tab has a loads column
      ...
      materialGridView()->disableFilter();
    }
  }
...
void MaterialGridController::addColumns(const QString &category, std::vector<QString> & fields)
{
  ...
  for(const auto &field : fields) {
    ...
    if ( field == NAME ) {
      ...
      connect(checkbox.data(), &QCheckBox::stateChanged, this, &MaterialGridController::selectAllStateChanged);
...
QString MaterialGridController::getColor(const model:: ModelObject & modelObject)
{
...
void MaterialGridController::checkSelectedFields()
{
...
void MaterialGridController::onItemDropped(const OSItemId& itemId)
{
...
void MaterialGridController::refreshModelObjects()
{
...
void MaterialGridController::onComboBoxIndexChanged(int index)
{
}
