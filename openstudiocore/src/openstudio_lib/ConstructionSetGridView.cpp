    ...
    m_isIP(isIP)
{
  ...
  auto spaceTypesGridController = new ConstructionSetGridController(m_isIP, "Space Types", IddObjectType::OS_SpaceType, model, spaceTypeModelObjects);
...
{
  auto gridView = this->constructionSetGridView();
  if (gridView) {
  ...
  if (constructionSetGridView()) {
    // Enable filter when viewing loads tab...
    if (index == 1) {
      constructionSetGridView()->enableFilter();
    }
    // ...or if the custom tab has a loads column
      ...
      constructionSetGridView()->disableFilter();
    }
  }
