    ...
    m_isIP(isIP)
{
  ...
  auto spaceTypesGridController = new ConstructionGridController(m_isIP, "Space Types", IddObjectType::OS_SpaceType, model, spaceTypeModelObjects);
...
{
  auto gridView = this->constructionGridView();
  if (gridView) {
  ...
  if (constructionGridView()) {
    // Enable filter when viewing loads tab...
    if (index == 1) {
      constructionGridView()->enableFilter();
    }
    // ...or if the custom tab has a loads column
      ...
      constructionGridView()->disableFilter();
    }
  }
